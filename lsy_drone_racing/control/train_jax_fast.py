...
HOVER_ARGS = dict(
    ...
    hidden_size=64,
    init_logstd=-1.0,
    hover_angvel_weight=0.05,
    act_coefs=(0.077, 0.077, 0.077, 0.077),
    d_act_coefs=(0.173, 0.173, 0.173, 0.173),
    num_last_actions=4,
)


def make_figure8_env(num_envs: int, device: str, args: Args) -> FlattenJaxObservation:
    ...
    env: FigureEightEnv = FigureEightEnv.create(
        ...
        device=device,
        reset_randomization=lambda data, mask: data,  # no extra reset randomization
    )
    env = NormalizeActions.create(env)
    env = ZeroYaw.create(env)
    env = AngleReward.create(env, weight=args.rpy_coef)
...
def make_hover_env(num_envs: int, device: str, args: Args) -> FlattenJaxObservation:
    """Build the hover (ReachPosEnv) environment on the attitude control interface."""
    env: ReachPosEnv = ReachPosEnv.create(
        ...
        reward_dist_scale=args.hover_reward_scale,
        reward_angvel_weight=args.hover_angvel_weight,
    )
    env = NormalizeActionsV2.create(env)
    env = ActionPenalty.create(
        env,
        num_actions=args.num_last_actions,
        init_last_actions=jp.array([[0.0, 0.0, 0.0, 0.0]]),
...
def make_env(
    task: str, num_envs: int, device: str, args: Args
) -> FlattenJaxObservation:
    """Dispatch to the task-specific environment builder."""
    return (
        make_figure8_env(num_envs, device, args)
        if task == "figure8"
        else make_hover_env(num_envs, device, args)
    )


def hover_pos_offset(env: FlattenJaxObservation) -> int:
    """Flat-observation offset of the (goal-relative) position for the hover metric.

    Observations are flattened in alphabetical key order; this returns the start index of "pos"
    in the pre-flatten Dict observation space.
    """
    dict_space = (
        env.base.single_observation_space
    )  # space just before FlattenJaxObservation
    offset = 0
    for k in sorted(dict_space.spaces.keys()):
        size = int(np.prod(dict_space.spaces[k].shape))
        if k == "pos":
            return offset
        offset += size
    raise KeyError("'pos' not found in observation space")
...
def collect_rollout(
    ...
    def step_once(carry: tuple, _: Any) -> tuple[tuple, tuple]:
        ...
        if args.task == "figure8":
            pos = env.unwrapped.data.states.pos[:, 0, :]
            dist = jp.linalg.norm(pos - env.unwrapped.target_pos, axis=-1)
        else:  # hover: pre-reset goal-relative position lives in info["final_obs"]
            rel_pos = info["final_obs"][:, args.pos_start : args.pos_start + 3]
...
def train_iteration(
    ...
    if args.task == "figure8":  # average deviation over all steps
        metric_sum, metric_count = jp.sum(dist), jp.asarray(dist.size, jp.float32)
    else:  # hover: final distance at episode end
        metric_sum, metric_count = (
            jp.sum(jp.where(done_now, dist, 0.0)),
            jp.sum(done_now),
        )
    metrics = {
        "ret_sum": ret_sum,
...
def train(args: Args) -> tuple[Agent, dict]:
    ...
    env = make_env(args.task, args.num_envs, args.device, args)
...
def evaluate(args: Args, agent: Agent, num_envs: int = 64) -> float:
    ...
    """
    env = make_env(args.task, num_envs, args.device, args)
    pos_start = hover_pos_offset(env) if args.task == "hover" else 0
...
def build_args(task: str, device: str, seed: int, timesteps: int | None) -> Args:
    ...
    pos_start = 0
    if args.task == "hover":
        probe = make_hover_env(1, "cpu", args)
