...
the raw, per-drone ``pos``/``target_gate`` and the env-shared ``gates_pos`` straight out of
``race_core.obs`` -- exactly the fields ``RelativeRacingObs`` drops or transforms into body-frame
features. Any position in the chain before that (after ``LogRewardComponents``, before/after
...
class CompetitionReward(Wrapper):
    ...
    Only supports ``n_drones == 2`` (ego + 1 opponent), matching the host-side ``OpponentWrapper``
    this is ported from.
    """
    ...
    def create(
        ...
        Args:
            base: The wrapped env, upstream of ``RelativeRacingObs`` in the chain (its ``obs`` dict
                must still expose the raw, per-drone ``pos``/``target_gate`` and the env-shared
                ``gates_pos``).
        ...
        def _apply(
            ...
            # gates_pos: (n_envs, n_gates, 3), shared across drones -- passed through as-is.
            ego_terminated = terminated[:, 0]
            components = competition_reward_fn(
                ego_pos, ego_gate, obs["gates_pos"], opp_pos, opp_gate, ego_terminated
            )
