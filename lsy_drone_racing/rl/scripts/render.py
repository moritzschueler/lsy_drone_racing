    ...
    **kwargs: Any,
    ...
    Args:
        ...
        ego_start_delay: Seconds the ego drone (0) hovers on its pad before its policy takes over.
            Unlike ``opponent_pid_start_frac`` (which *teleports* the opponent ahead), the
            opponent(s) here fly normally from the pad during the delay, building a genuine head
            start -- whether they're the scripted PID (``opponent == "pid"``) or a mirrored
            checkpoint (``opponent == "self_play"``). The ego is held in place by a separate
            attitude PID targeting its own start pad, so this needs an attitude-control track
            config (``control_mode == "attitude"``) regardless of ``opponent``.
    ...
    assert ego_start_delay == 0.0 or env_param(env_config, "control_mode") == "attitude", (
        "ego_start_delay holds the ego with an attitude-PID hover-hold and needs an "
        f"attitude-control track config, got control_mode="
        f"'{env_param(env_config, 'control_mode')}'."
    )
    traj_pid = None
    ego_pid = None  # attitude PID that hovers drone 0 on its pad during ``ego_start_delay``
    # Built once, shared by both the ``opponent == "pid"`` trajectory-follower and the ego hover-
    # hold PID below -- either one needs it, so compute it at most once regardless of which (or
    # both) are active.
    drone_mass = action_space = None
    if opponent == "pid" or ego_start_delay > 0.0:
    ...
    if ego_start_delay > 0.0:
        assert n_drones > 1, "ego_start_delay needs a multi-drone task/config to give a head start."
    ...
    @nnx.jit
    def policy_step_self_play_hold(
        agent: Agent, env: Wrapper, obs: Array, ego_hold: Array, ego_i_error: Array
    ) -> tuple[Wrapper, Array, Array, Array, Array]:
        """Like ``policy_step``, but drone 0's action is a pad hover-hold while ``ego_hold``.

        Mirrors ``policy_step_pid``'s ego-hold branch, but opponents (drones ``1..``) stay
        self-play (the mirrored ego checkpoint) instead of switching to the scripted PID -- so a
        checkpoint opponent can also be given a genuine head start via ``ego_start_delay``. Only
        used when ``ego_pid is not None`` (see its construction above).
        """
        mean, _, _ = agent(obs)
        states = env.unwrapped.data.sim_data.states
        ego_pos, ego_vel, ego_quat = states.pos[:, 0], states.vel[:, 0], states.quat[:, 0]
        ...
        ego_action = jnp.where(ego_hold, ego_pid.normalize(hold_phys), mean[:, 0])
        action = mean.at[:, 0].set(ego_action)
        env, (obs, _, terminated, truncated, _) = env.step(env, action)
        return env, obs, terminated, truncated, ego_i_error

    eval_env, (obs, _) = eval_env.reset(eval_env, seed=args.seed)
    if ego_start_delay > 0.0:
        # Hold-only PID for the ego: same gains/attitude convention, but seeded at its actual
        # post-reset position AND yaw rather than the config's nominal pad -- with spawn
        # randomization (drone_pos/drone_rpy) enabled these can differ, and holding against the
        # nominal pad/yaw would drag the drone off its real (randomized) start. The PID always
        # regulates to des_yaw (0 by default, the racing convention), so a randomized nonzero spawn
        # yaw was otherwise fought as an "error": the hold spent its first steps rotating back to 0,
        # and the roll/pitch demand -- computed assuming the body is already at des_yaw -- was
        # briefly wrong while it wasn't, showing up as lateral drift. Pointing des_yaw at the actual
        # spawn yaw makes the hold target wherever the drone actually is, attitude included. Driven
        # at virtual_t=0, speed=0 it targets that position with zero velocity -> a hover hold.
        # Waypoints/gates are irrelevant here (never advanced past t=0), so gates=None. This is
        # independent of ``opponent`` -- it works whether drones 1.. are the scripted PID or a
        # mirrored self-play checkpoint.
        ego_start_pos = np.asarray(eval_env.unwrapped.data.sim_data.states.pos[0, 0])
        ego_start_quat = np.asarray(eval_env.unwrapped.data.sim_data.states.quat[0, 0])
        ego_start_yaw = float(R.from_quat(ego_start_quat).as_euler("xyz")[2])
    ...
    finish_times = np.full(n_drones, np.nan)  # sim time (s) each drone stopped being alive
    if sim is not None:
    ...
    if ego_pid is not None:
        opponent_desc = "PID opponent" if traj_pid is not None else "self-play opponent"
        print(
            f"Ego waits {ego_start_delay:.1f}s ({ego_hold_steps} steps) on its pad before racing; "
            f"the {opponent_desc} flies from the pad meanwhile."
        )
    ...
    while not done:
        ...
        elif ego_pid is not None:
            ego_hold = jnp.asarray(step < ego_hold_steps)
            eval_env, obs, terminated, truncated, ego_i_error = policy_step_self_play_hold(
                agent, eval_env, jnp.asarray(obs), ego_hold, ego_i_error
            )
        else:
        ...
        newly_done = alive & done_per_drone
        finish_times[newly_done] = (step + 1) / env_param(env_config, "freq")
    ...
    for drone in range(n_drones):
        label = "ego" if drone == 0 else f"opponent {drone}"
        line = f"Drone {drone} ({label}): stopped at t={finish_times[drone]:.2f}s"
        if drone == 0 and ego_start_delay > 0.0:
            # The ego hovers on its pad for ego_start_delay before racing, so its raw stopped-at
            # time isn't comparable to the other drones' racing speed -- report flight time too.
            flight_time = finish_times[drone] - ego_start_delay
            line += f" (flight time {flight_time:.2f}s after {ego_start_delay:.2f}s hold)"
        print(line)
