...
VecDroneRaceEnv          # raw dict obs: pos, quat, vel, ang_vel, target_gate,
  ...
  → AngleReward          # orientation penalty (no obs change)
  → ActionPenalty        # adds `last_action` (4) to the dict
...
| `last_action` | (4,) | 4 | Previous action `[roll, pitch, yaw, thrust]`; yaw is always 0 |
