"""Unit tests for the champion-paper gate distance / progress reward and the speed barrier."""
...
def test_distance_is_unbounded_with_constant_gradient():
    """Unlike the old bounded potential, distance grows linearly and its gradient never saturates."""
    assert _dist(-2.8) == pytest.approx(2.8, abs=1e-6)  # straight-on distance == |along|
    # Equal steps in along give equal distance changes near and far from the gate (no vanishing pull).
...
def test_build_reward_constructs_for_any_positive_coefficients():
    ...
    crossing drop), so any sensible coefficients build a reward fn."""
...
def test_action_smoothness_uses_the_bounded_action():
    """Out-of-range commands are clipped to [-1, 1] before differencing, so an unbounded high-variance
    policy cannot inflate the penalty -- it saturates at the bounded value."""
    a_prev = jnp.zeros((1, 4))
...
@pytest.mark.unit
def test_speed_barrier_diverges_toward_max_speed_but_stays_finite():
    """The speed penalty is 0 at rest, exponentially increasing, and a bounded (finite) wall."""
    vmax, slope = 3.0, 0.3
    speeds = jnp.array([0.0, 1.0, 1.5, 2.0, 2.5, 2.9])  # up to just below the wall
    pen = np.asarray(soft_speed_penalty(speeds, vmax, slope))
    assert pen[0] == 0.0  # free at rest
    assert np.all(np.diff(pen) > 0)  # strictly increasing with speed
    assert np.all(np.diff(np.diff(pen)) > 0)  # convex: steepening toward the wall (exponential)
    # Below the limit it is modest; near the wall it is already large (a firm ceiling). These are
    # unweighted ramp values (the caller multiplies by speed_coef ~ 0.05).
    assert float(soft_speed_penalty(jnp.array(0.5 * vmax), vmax, slope)) < 0.5
    assert float(soft_speed_penalty(jnp.array(0.9 * vmax), vmax, slope)) > 5.0
    # Clamped: at and beyond max_speed it saturates to a large-but-finite value (no inf/nan).
    wall = soft_speed_penalty(jnp.array([vmax, 2.0 * vmax, 100.0]), vmax, slope)
    assert np.all(np.isfinite(np.asarray(wall)))
    assert np.allclose(np.asarray(wall), float(wall[0]))  # saturated to the same cap past max_speed
